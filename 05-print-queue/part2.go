...
func FixInvalidUpdate(rules map[int][]int, update []int) []int {
	numToIndexMap := make(map[int]int)
	for i, num := range update {
		numToIndexMap[num] = i
	}
	...
	sort.Slice(arr, func(i, j int) bool {
		...
		rulesForRight, ok := rules[right]

		return ok && slices.Contains(rulesForRight, left)
	})
...
func main() {
	...
	for _, update := range updates {
		if !IsValidUpdate(rules, update) {
			fixedUpdate := FixInvalidUpdate(rules, update)
			num := GetMiddlePageNumber(fixedUpdate)
			total += num
		}
	}
